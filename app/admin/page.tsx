...
export default function AdminPage() {
  ...
  return <main className={styles.shell}>
    <aside className={styles.sidebar}>
      <a className={styles.brand} href="/"><span className={styles.seal}>HIU</span><span><strong>ADMIN CENTER</strong><small>HIU YHCT ECOSYSTEM</small></span></a>
      <p className={styles.sideLabel}>KHÔNG GIAN QUẢN TRỊ</p>
      <nav className={styles.nav} aria-label="Mục quản trị">{tabList.map((item) => <button key={item.id} className={tab === item.id ? styles.selected : ""} onClick={() => { setTab(item.id); setNotice(""); }}><span>{item.icon}</span>{item.title}</button>)}</nav>
      <a className={styles.back} href="/">← Xem trang chính</a>
    </aside>
    <section className={styles.main}>
      <header className={styles.top}><div><span className={styles.eyebrow}>HIU YHCT · BẢNG ĐIỀU KHIỂN</span><h1>{tabList.find((item) => item.id === tab)?.title}</h1></div><span className={styles.localTag}>BẢN NHÁP CỤC BỘ</span></header>
      <div className={styles.warning}><strong>Chỉ lưu trên trình duyệt này</strong><p>Chưa có xác thực Admin, phân quyền máy chủ hoặc backend xuất bản dùng chung. Các chỉnh sửa không làm thay đổi nội dung đang công khai và không đồng bộ giữa thành viên.</p></div>
      {notice && <p className={styles.statusMessage} role="status">{notice}</p>}

      {tab === "overview" && <div className={styles.content}><div className={styles.metrics}><article><small>HUB</small><strong>{ecosystemApps.length}</strong><span>4 điểm đến trong registry</span></article><article><small>BẢN NHÁP ĐÃ SỬA</small><strong>{Object.keys(drafts).length}</strong><span>Thiết bị hiện tại</span></article><article><small>MỤC CHỜ DUYỆT</small><strong>{stored.queue.filter((x) => x.status === "Chờ duyệt").length}</strong><span>Hàng chờ cục bộ</span></article><article><small>NHẬT KÝ</small><strong>{stored.logs.length}</strong><span>Trên trình duyệt</span></article></div><section className={styles.panel}><h2>Quản lý lối vào hệ sinh thái</h2><p>Chỉnh tên, mô tả, URL HTTPS; bản nháp áp dụng lên bản đồ, thẻ ứng dụng và các lối vào nhanh trên cùng trình duyệt.</p><button className={styles.primary} onClick={() => setTab("content")}>Mở Nội dung Hub →</button></section><section className={styles.panel}><h2>Quy trình duyệt</h2><p>Admin và Moderator là khung giao diện. Chưa có người dùng hoặc quyền nào được xác lập tại máy chủ.</p><button className={styles.secondary} onClick={() => setTab("roles")}>Xem vai trò & quyền</button></section></div>}
      ...
      {tab === "roles" && <div className={styles.content}><div className={styles.intro}><h2>Thành viên và vai trò</h2><p>Hiện chưa kết nối dữ liệu thành viên hoặc hệ thống đăng nhập.</p></div><div className={styles.roles}><article><span>◇</span><h3>Admin</h3><p>Vai trò dự kiến: quản lý nội dung Hub, cấu hình và quy trình xuất bản.</p><b>Chưa có xác thực máy chủ</b></article><article><span>◎</span><h3>Moderator</h3><p>Vai trò dự kiến: xem xét và gửi đề xuất nội dung cho Admin.</p><b>Chưa có phân quyền máy chủ</b></article></div><div className={styles.warning}><strong>Không có hồ sơ thành viên thật</strong><p>Không tự tạo tên, email, điểm số hoặc thông tin tài khoản.</p></div></div>}
