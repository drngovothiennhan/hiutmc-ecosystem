...
export default function EcosystemMap() {
  ...
  return (
    <div className="heritageMapWrap">
      <div className="mapStage heritageMapStage" aria-label="Bản đồ học viện HIU: bốn khu vực học tập">
        ...
        {ecosystemApps.map((app) => (
          <div className={`districtMarker district-${app.slug}`} key={app.slug} style={{left:`${app.x}%`,top:`${app.y}%`,["--accent" as string]:app.accent}}>
            <a className="districtBanner" href={app.launchUrl} data-app-transition aria-label={`Mở ${app.name}`}><strong>{app.name}</strong><small>{app.tagline}</small></a>
          </div>
        ))}
      ...
      <nav className="quickDock heritageQuickDock" aria-label="Vào nhanh bốn ứng dụng">{ecosystemApps.map(app=><a href={app.launchUrl} data-app-transition key={app.slug}>{app.shortName} <span aria-hidden="true">↗</span></a>)}</nav>
    </div>
  );
}
